...
#######################################
#----- Shnnon Diversity - Carbohydrate
#######################################
#---- Carbohydrate Metabolism - Human
Color <- c("skyblue4", "skyblue")
metadata_human <- read.csv (file = "Final_Data/Metadata_Human.txt", row.names=1, header = T, sep = "\t")
Groups <- metadata_human[,2:3]
Fam <- read.csv(file = "Final_Data/Fam_ABUNDANCE", row.names=1, header = T, sep = "\t")
Fam <- data.frame(t(Fam))
Fam_human <- Fam[24:51,]
N.obs_carbo_human <- rowSums(Fam_human > 0)
H_carbo_human <- diversity(Fam_human)
Simp_carbo__human <- diversity(Fam_human, "invsimpson")
div_carbo_human <- data.frame(N.obs_carbo_human, H_carbo_human, Simp_carbo__human, Groups)
jpeg("Carbo_Shannon_Human.jpg", height = 3, width = 2, units = 'in', res = 600)
ggplot(aes(y = H_carbo_human, x = Group1, fill = Group1), data = div_carbo_human) + geom_boxplot() + ggtitle("Carbo degradation") + labs(x="",y="Shannon's H") + theme_classic() + scale_color_manual(values=Color) + scale_fill_manual(values=Color) + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + theme(legend.text = element_text(colour="black", size=8)) + theme(legend.title = element_blank()) + theme(axis.text.x = element_text(size = 8, colour = "black", face = "bold"), axis.text.y = element_text(size = 8, colour = "black", face = "bold"))
dev.off ()
wilcox.test(div_carbo_human$H_carbo_human ~ div_carbo_human$Group1, p.adjust.method="fdr")
#W = 180, p-value = 4.532e-05

#---- Carbohydrate Metabolism - Gorilla
Color <- c("wheat4", "wheat")
metadata_gorilla <- read.csv (file = "Final_Data/Metadata_Gorilla.txt", row.names=1, header = T, sep = "\t")
Groups <- metadata_gorilla[,2:3]
Fam_Gorilla <- Fam[1:23,]
N.obs_carbo_gorilla <- rowSums(Fam_Gorilla > 0)
H_carbo_goirlla <- diversity(Fam_Gorilla)
Simp_carbo_gorilla <- diversity(Fam_Gorilla, "invsimpson")
div_carbo_gorilla <- data.frame(N.obs_carbo_gorilla, H_carbo_goirlla, Simp_carbo_gorilla, Groups)
jpeg("Carbo_Shannon_Gorilla.jpg", height = 3, width = 2, units = 'in', res = 600)
ggplot(aes(y = H_carbo_goirlla, x = Group1, fill = Group1), data = div_carbo_gorilla) + geom_boxplot() + ggtitle("Carbo degradation") + labs(x="",y="Shannon's H") + theme_classic() + scale_color_manual(values=Color) + scale_fill_manual(values=Color) + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + theme(legend.text = element_text(colour="black", size=8)) + theme(legend.title = element_blank()) + theme(axis.text.x = element_text(size = 8, colour = "black", face = "bold"), axis.text.y = element_text(size = 8, colour = "black", face = "bold"))
dev.off ()
wilcox.test(div_carbo_gorilla$H_carbo_goirlla ~ div_carbo_gorilla$Group1, p.adjust.method="fdr")
#W = 93, p-value = 0.1037

#--- Figure3C and D:
#######################################
#----- Shannon Diversity -Fat metabolism
#######################################
#---- Fat Metabolism - Human
Color <- c("skyblue4", "skyblue")
Pathway <- read.csv(file = "Lipid_metabolism//Lipid_metabolism_raw.txt", row.names=1, header = T, sep = "\t")
Pathway <- data.frame(t(Pathway))
Pathway_human <- Pathway[24:51,]
N.obs_fat_human <- rowSums(Pathway_human > 0)
H_fat_human <- diversity(Pathway_human)
Simp_fat_human <- diversity(Pathway_human, "invsimpson")
div_fat_human <- data.frame(N.obs_fat_human, H_fat_human, Simp_fat_human, metadata_human)
jpeg("Fat_Shannon_Human.jpg", height = 3.3, width = 2.5, units = 'in', res = 600)
ggplot(aes(y = H_fat_human, x = Group1, fill = Group1), data = div_fat_human) + geom_boxplot() + ggtitle("Fat degradation") + labs(x="",y="Shannon's H") + theme_classic() + scale_color_manual(values=Color) + scale_fill_manual(values=Color) + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + theme(legend.text = element_text(colour="black", size=8)) + theme(legend.title = element_blank()) + theme(axis.text.x = element_text(size = 10, colour = "black", face = "bold"), axis.text.y = element_text(size = 10, colour = "black", face = "bold"))
dev.off ()
wilcox.test(div_fat_human$H_fat_human ~ div_fat_human$Group1, p.adjust.method="fdr")
#W = 147, p-value = 0.02412

#---- Fat Metabolism - Gorilla
Color <- c("wheat4", "wheat")
Pathway_gorilla <- Pathway[1:23,]
N.obs_fat_gorilla <- rowSums(Pathway_gorilla > 0)
H_fat_gorilla <- diversity(Pathway_gorilla)
Simp_fat_gorilla <- diversity(Pathway_gorilla, "invsimpson")
div_fat_gorilla <- data.frame(N.obs_fat_gorilla, H_fat_gorilla, Simp_fat_gorilla, metadata_gorilla)
jpeg("Fat_Shannon_Gorilla.jpg", height = 3.3, width = 2.5, units = 'in', res = 600)
ggplot(aes(y = H_fat_gorilla, x = Group1, fill = Group1), data = div_fat_gorilla) + geom_boxplot() + ggtitle("Fat degradation") + labs(x="",y="Shannon's H") + theme_classic() + scale_color_manual(values=Color) + scale_fill_manual(values=Color) + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + theme(legend.text = element_text(colour="black", size=8)) + theme(legend.title = element_blank()) + theme(axis.text.x = element_text(size = 10, colour = "black", face = "bold"), axis.text.y = element_text(size = 10, colour = "black", face = "bold"))
dev.off ()
wilcox.test(div_fat_gorilla$H_fat_gorilla ~ div_fat_gorilla$Group1, p.adjust.method="fdr")
#W = 44, p-value = 0.1896
...
#---- Carbo Diversity
#####################
jpeg("Carbo_fat_New.jpg", height = 3, width = 4, units = 'in', res = 600)
ggplot(data = Complete_Table_filtered, mapping = aes(x = Carbo_Shannon, y = Fat_Shannon)) + 
  geom_point(mapping = aes(color = Group1)) + scale_color_manual(values=Colors) + 
   theme_bw() +
  xlab("Diversity (Carbohydrate metabolism)") + ylab("Diversity Fat metabolism)") +
  geom_smooth(method=lm, color="darkgray", fill="gray")
dev.off ()
Correlation <- corr.test(Complete_Table_filtered$Carbo_Shannon, Complete_Table_filtered$Fat_Shannon, method="spearman", adjust="fdr", alpha =0.5, use = "pairwise")
...
##########################################
jpeg("Carbo_fat_Relab.jpg", height = 3, width = 4, units = 'in', res = 600)
ggplot(data = Complete_Table_filtered, mapping = aes(x = Carbo_totalSum, y = Fat_totalSum)) + 
  geom_point(mapping = aes(color = Group1)) + scale_color_manual(values=Colors) + 
  theme_bw() +
  xlab("Sqrt abundance (carbo meta) %") + ylab("Sqrt abundance (fat meta) %") +
  geom_smooth(method=lm, color="darkgray", fill="gray")
dev.off ()
Correlation <- corr.test(Complete_Table_filtered$Carbo_totalSum, Complete_Table_filtered$Fat_totalSum, method="spearman", adjust="fdr", alpha =0.5, use = "pairwise")
